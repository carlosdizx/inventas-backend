...
public interface FacturaDao extends CrudRepository<Factura, Integer>
{
    ...
    @Query(value = "SELECT new arena.models.FacturaInfo(f.id,f.descripcion,f.fecha,p.nombre,p.precioVenta,if.cantidad,p.precioVenta*if.cantidad) FROM Factura f " +
            "INNER JOIN ItemFactura if on f.id= if.factura.id " +
            "INNER JOIN Producto p on p.id = if.producto.id " +
            "GROUP BY f.id,f.descripcion,f.fecha, p.nombre, p.precioVenta, if.cantidad " +
            "ORDER BY f.id")
